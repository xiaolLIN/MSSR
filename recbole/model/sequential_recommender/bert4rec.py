...
class BERT4Rec(SequentialRecommender):
    ...
    def calculate_loss(self, interaction):
        ...
        seq_output = self.forward(masked_item_seq)          # [B, max_len, H]
        ...
        seq_output = torch.bmm(pred_index_map, seq_output)  # [B mask_len H]   只取出mask位置的表示。
        ...
        if self.loss_type == 'BPR':
            ...
            # logits.view(-1, test_item_emb.size(0))  logits [B*mask_len, item_num]
            # pos_items.view(-1)                   pos_items [B*mask_len]
