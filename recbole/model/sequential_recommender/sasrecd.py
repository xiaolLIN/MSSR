...
class SASRecD(SequentialRecommender):
    ...
    def forward(self, item_seq, item_seq_len):
        ...
        for feature_embed_layer in self.feature_embed_layer_list:
            sparse_embedding, dense_embedding = feature_embed_layer(None, item_seq)  # can be seen as a embedding table?
        ...
        trm_output = self.trm_encoder(input_emb, feature_emb, position_embedding, extended_attention_mask,
        ...
        if self.loss_type == 'BPR':
            ...
            if self.attribute_predictor!='' and self.attribute_predictor!='not':
                ...
                for i, a_predictor in enumerate(self.ap):
                    ...
                    # print('interaction.interaction:', interaction.interaction)

                    # print('before one hot:', attribute_labels.shape)
                    attribute_labels = nn.functional.one_hot(attribute_labels, num_classes=self.n_attributes[
                        self.selected_features[i]])  # [bs, 14, 355] 355 is the nums of features
                    # print('after one hot:', attribute_labels.shape)

                    if len(attribute_labels.shape) > 2:
                        attribute_labels = attribute_labels.sum(dim=1)
                    attribute_labels = attribute_labels.float()  # [bs, 355]
                    # print(attribute_labels.shape)       # [bs, 355]
