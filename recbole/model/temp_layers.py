...
class CrossMultiHeadAttention(nn.Module):
    ...
    def __init__(self, n_heads, hidden_size,attribute_hidden_size,feat_num, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps,fusion_type,max_len,init_type):
        ...
        if self.init_type == 'not':
            self.w_matrix = torch.ones(1,9)
        elif self.init_type == 'one':
            w_matrix = torch.empty(1,9)
            self.w_matrix = torch.nn.init.constant_(w_matrix, val=1.0)
            self.w_matrix = nn.Parameter(self.w_matrix, requires_grad=True)
            ...
            w_matrix = torch.empty(1,9)
            self.w_matrix = torch.nn.init.uniform_(w_matrix, a=0, b=1)
            self.w_matrix = nn.Parameter(self.w_matrix, requires_grad=True)
        self.w_matrix.data = torch.softmax(self.w_matrix, dim=-1)
    ...
    def forward(self, input_tensor,attribute_table,position_embedding, attention_mask):
        ...
        item_attention_scores = item_attention_scores / math.sqrt(self.attention_head_size)
        item_attention_scores = item_attention_scores + attention_mask
        item_attention_scores = nn.Softmax(dim=-1)(item_attention_scores)

        i_p_attention_scores = i_p_attention_scores / math.sqrt(self.attention_head_size)
        i_p_attention_scores = i_p_attention_scores + attention_mask
        i_p_attention_scores = nn.Softmax(dim=-1)(i_p_attention_scores)

        i_attribute_attention_scores = i_attribute_attention_scores / math.sqrt(self.attention_head_size)
        i_attribute_attention_scores = i_attribute_attention_scores + attention_mask
        i_attribute_attention_scores = nn.Softmax(dim=-1)(i_attribute_attention_scores)

        p_i_attention_scores = p_i_attention_scores / math.sqrt(self.attention_head_size)
        p_i_attention_scores = p_i_attention_scores + attention_mask
        p_i_attention_scores = nn.Softmax(dim=-1)(p_i_attention_scores)

        pos_scores = pos_scores / math.sqrt(self.attention_head_size)
        pos_scores = pos_scores + attention_mask
        pos_scores = nn.Softmax(dim=-1)(pos_scores)

        p_attribute_attention_scores = p_attribute_attention_scores / math.sqrt(self.attention_head_size)
        p_attribute_attention_scores = p_attribute_attention_scores + attention_mask
        p_attribute_attention_scores = nn.Softmax(dim=-1)(p_attribute_attention_scores)

        attribute_i_attention_scores = attribute_i_attention_scores / math.sqrt(self.attention_head_size)
        attribute_i_attention_scores = attribute_i_attention_scores + attention_mask
        attribute_i_attention_scores = nn.Softmax(dim=-1)(attribute_i_attention_scores)

        attribute_p_attention_scores = attribute_p_attention_scores / math.sqrt(self.attention_head_size)
        attribute_p_attention_scores = attribute_p_attention_scores + attention_mask
        attribute_p_attention_scores = nn.Softmax(dim=-1)(attribute_p_attention_scores)

        attribute_attention_scores = attribute_attention_scores / math.sqrt(self.attention_head_size)
        attribute_attention_scores = attribute_attention_scores + attention_mask
        attribute_attention_scores = nn.Softmax(dim=-1)(attribute_attention_scores)
        ...
        p_i_attention_scores = p_i_attention_scores * self.w_matrix[0][3]
        pos_scores = pos_scores * self.w_matrix[0][4]
        p_attribute_attention_scores = p_attribute_attention_scores * self.w_matrix[0][5]
        attribute_i_attention_scores = attribute_i_attention_scores * self.w_matrix[0][6]
        attribute_p_attention_scores = attribute_p_attention_scores * self.w_matrix[0][7]
        attribute_attention_scores = attribute_attention_scores * self.w_matrix[0][8]
        attention_probs = attribute_attention_scores + item_attention_scores + pos_scores + attribute_i_attention_scores +\
...
class CrossTransformerLayer(nn.Module):
    ...
    def __init__(
        self, n_heads, hidden_size,attribute_hidden_size,feat_num, intermediate_size, hidden_dropout_prob, attn_dropout_prob, hidden_act,
        layer_norm_eps,fusion_type,max_len, init_type
    ):
        super(CrossTransformerLayer, self).__init__()
        self.multi_head_attention = CrossMultiHeadAttention(
            n_heads, hidden_size,attribute_hidden_size, feat_num, hidden_dropout_prob, attn_dropout_prob, layer_norm_eps,fusion_type,max_len,init_type
        )
        self.feed_forward = FeedForward(hidden_size, intermediate_size, hidden_dropout_prob, hidden_act, layer_norm_eps)
...
class CrossTransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        layer = CrossTransformerLayer(
            ...
            hidden_act, layer_norm_eps, fusion_type, max_len, init_type
        )
        self.layer = nn.ModuleList([copy.deepcopy(layer) for _ in range(n_layers)])
